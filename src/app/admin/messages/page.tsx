            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="p-8 text-center">
                  <MessageSquare className="w-10 h-10 text-portland-gray/30 mx-auto mb-3" />
                  <p className="text-sm text-portland-gray">No conversations yet</p>
                </div>
              ) : (
                filteredThreads.map((thread) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 border-b border-portland-light/50 hover:bg-portland-light/30 transition-colors ${
                        selectedThread?.id === thread.id ? "bg-portland-light/50" : ""
                      }`}
                    >
                      <div className="flex items-start justify-between mb-1">
                        <span className="text-sm font-semibold text-portland-dark truncate">{thread.subject}</span>
                        {thread.unreadCount > 0 && (
                          <span className="ml-2 shrink-0 w-5 h-5 bg-portland-red text-white text-xs font-bold rounded-full flex items-center justify-center">
                            {thread.unreadCount}
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-portland-gray mb-1">
          ...
          {selectedThread ? (
            <Card padding={false} className="h-full flex flex-col">
              ...
              <div className="p-4 border-b flex items-center justify-between">
                ...
                <button onClick={() => setSelectedThread(null)} className="p-2 hover:bg-portland-light rounded-lg">
                  <X className="w-4 h-4 text-portland-gray" />
                </button>
              </div>
                        ...
                        <div className={`max-w-[70%] ${msg.senderId === threadMessages.find((m) => m.senderId !== msg.senderId)?.senderId ? "" : ""}`}>
                          <div className={`rounded-2xl px-4 py-2.5 ${
                            msg.senderId === threadMessages.find((m, i) => i > 0 && m.senderId !== msg.senderId)?.senderId || (threadMessages.indexOf(msg) === 0 && msg.senderId !== threadMessages[1]?.senderId)
                              ? "bg-portland-light text-portland-dark"
                              : "bg-portland-red text-white"
                          }`}>
                            <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                          </div>
                          <p className="text-[10px] text-portland-gray/60 mt-1 px-2">
              ...
              <form onSubmit={handleSend} className="p-4 border-t flex gap-3">
                <input
                  ...
                  className="flex-1 bg-portland-light rounded-xl px-4 py-2.5 text-sm text-portland-dark placeholder:text-portland-gray/50 focus:outline-none focus:ring-2 focus:ring-portland-red/30"
                  disabled={sending}
                />
            ...
            <Card className="h-full flex items-center justify-center">
              <div className="text-center">
                <MessageSquare className="w-16 h-16 text-portland-gray/20 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-portland-dark mb-1">Select a conversation</h3>
                <p className="text-sm text-portland-gray">Choose a conversation from the list or start a new one.</p>
              </div>
            </Card>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowNewThread(false)}>
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b flex items-center justify-between">
              ...
              <button onClick={() => setShowNewThread(false)} className="p-1 hover:bg-portland-light rounded-lg">
                <X className="w-5 h-5 text-portland-gray" />
              </button>
            </div>
            <form onSubmit={handleCreateThread} className="p-6 space-y-4 overflow-y-auto max-h-[60vh]">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {threadParticipants.map((userId) => {
                    ...
                    return u ? (
                      <span key={userId} className="inline-flex items-center gap-1 bg-portland-red/10 text-portland-red px-2.5 py-1 rounded-full text-xs font-medium">
                        {u.name || u.email}
                  ...
                  <div className="mt-2 max-h-40 overflow-y-auto border rounded-lg">
                    {filteredUsers.filter((u) => !threadParticipants.includes(u.id)).slice(0, 10).map((u) => (
                      <button
                        ...
                        className="w-full text-left p-3 hover:bg-portland-light flex items-center gap-3 border-b last:border-0"
                      >
                        <div className="w-8 h-8 bg-portland-red/10 rounded-full flex items-center justify-center">
                          <span className="text-portland-red text-xs font-bold">{(u.name || u.email).charAt(0).toUpperCase()}</span>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-portland-dark">{u.name || "No name"}</p>
                          <p className="text-xs text-portland-gray">{u.email} · {ROLE_LABELS[u.role] || u.role}</p>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-portland-light rounded-xl px-4 py-3 text-sm text-portland-dark placeholder:text-portland-gray/50 focus:outline-none focus:ring-2 focus:ring-portland-red/30 resize-none"
                  required
                />
              </div>
